...
const pool = new Pool({
  connectionString:
    ...
    'postgres://appuser:apppassword@localhost:5432/appdb'
});
...
app.get('/orders', async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT id, user_id, item, price FROM orders ORDER BY id'
    );
...
app.get('/orders/:id', async (req, res) => {
  try {
    const orderResult = await pool.query(
      'SELECT id, user_id, item, price FROM orders WHERE id = $1',
      [req.params.id]
    );
    ...
    try {
      const userResponse = await fetch(
        `${USER_SERVICE_URL}/users/${order.user_id}`
      );

      userData = userResponse.ok
...
app.post('/orders', async (req, res) => {
  ...
  if (!user_id || !item || price == null) {
    return res.status(400).json({
      error: 'user_id, item, and price are required'
    });
  }
